...
function drawVelocityVector(x, y) {
    ...
    let vectorAngle = Math.atan2(endY - startY, endX - startX);

    ctx.beginPath();
    ctx.moveTo(endX, endY);

    ctx.lineTo(
        endX - 8 * Math.cos(vectorAngle - 0.3),
        endY - 8 * Math.sin(vectorAngle - 0.3)
    );
