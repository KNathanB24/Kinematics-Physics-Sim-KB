...
function updateEnergy(x, y) {
    ...
    keBar.style.width = Math.min(100, kePercent) + "%";
    peBar.style.width = Math.min(100, pePercent) + "%";
    teBar.style.width = Math.min(100, tePercent) + "%";
}
...
function drawAxes() {
    ...
    let stepMeters =
        Math.max(5, Math.round((50 / scale) / 5) * 5);
